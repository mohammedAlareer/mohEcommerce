              ...
              <span className="text-gray-500">Color</span>
                <div className="flex items-center gap-2">
                  {product.colors.map((color) => (
                    <div key={color} className={`cursor-pointer border-1 rounded-full p-[1.2] ${productTypes.color === color ? 'border-gray-600' :"border-gray-300"}`} onClick={() => handleProductType({type:"color" ,value:color })}>
                        <div className='w-[14] h-[14] rounded-full' style={{backgroundColor: color}}></div>
                    </div>
                  ))}
                  </div>
            </div>
